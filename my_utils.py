...
#    figure=ax.scatter(x_par, y_par, c=c_par, vmin=c_min,vmax=c_max,alpha=0.4,edgecolor='None',cmap=cm)
    ...
    if (if_scatter==1):
        figure=ax.scatter(x_par, y_par, s=10, c=den_par,                          vmin=0,vmax=1,alpha=0.25,edgecolor='lightyellow',cmap=cm,                          rasterized=True,label=title)
        figure=ax.scatter(x_par, y_par, s=12, c=den_par,                          vmin=0,vmax=1,alpha=0.25,edgecolor='None',cmap=cm,                          rasterized=True)

    if (if_contour==1):
        p_cont2=ax.contourf(xbins[0:nbins],ybins[0:nbins],flip_counts_rot,                             levels_cont,cmap=cm_r,alpha=0.8,linewidths=2)
...
#
#    print("# den_par_min=",den_par_min)
    mask_SFGs = mask_SFGs & (den_par<den_par_min) &     np.logical_not(np.isnan(x_par)) & np.isfinite(x_par) &     np.logical_not(np.isnan(y_par)) & np.isfinite(y_par)  
...
#    figure=ax.scatter(x_par, y_par, c=den_par, vmin=0,vmax=1,alpha=0.4,edgecolor='None',cmap=cm)
    ...
    m1, sM1, sD1 = binning_M(x_sf, y_sf, bin1 , min1 , max1, N_min, bin_size, bin_number )
    if (if_bin==1):
        p1 = ax.errorbar(m1, sM1, yerr= sD1, markersize = 10,                         markerfacecolor =c_color2, markeredgecolor = 'black',                          fmt = 'o', ecolor='black', elinewidth = 1, label = '', zorder=2, alpha=1.0)
...
#        x_plot=np.linspace(np.min(m1)-1.5*bin1, np.max(m1)+1.5*bin1, 10)
        ...
        y_plot=funct(x_plot,pa1[0],pa1[1])
        if (if_plot==1):
            p_fit, =ax.plot(x_plot, y_plot, '-', linewidth = 3, markerfacecolor =c_color  ,                            color =c_color, zorder=3, alpha=0.8)
...
#    figure=ax.scatter(x_par, y_par, c=den_par, vmin=0,vmax=1,alpha=0.4,edgecolor='None',cmap=cm)
    ...
    m1, sM1, sD1 = binning_M(x_sf, y_sf, bin1 , min1 , max1, N_min, bin_size, bin_number )
    if (if_bin==1):
        p1 = ax.errorbar(m1, sM1, yerr= 0.5*sD1, markersize = 10,                         markerfacecolor =c_color2, markeredgecolor = 'black',                          fmt = 'o', ecolor='black', elinewidth = 1, label = '', zorder=2, alpha=1.0)
...
def plot_2Dhist_new(x_par=None,y_par=None,z_par=None,x_min=-25,x_max=-16,y_min=0,y_max=4.5,z_min=-2,z_max=10,c_main='grey',label='MPL-11',                x_label='NSA z-band abs mag',y_label='u-z mag',figname='CMD_diag',alpha=0.75,                   size_scatter=5.0,n_zbins=11,labels_zbins=None,color_cm_now='coolwarm_r',z_lim=80,               x_loc=0.02,y_loc=0.03,z_label='',reverse=0, xscale='linear', yscale='linear'):
...
#    colormap = plt.cm(color_cm_now)
    ax.scatter(x_par, y_par, s=size_scatter*3, c=z_par,               vmin=z_min,vmax=z_max,alpha=alpha,edgecolor='None',cmap=color_cm_now,               rasterized=True,label=label)
    my_contour(ax,x_par,y_par,x_min,x_max,y_min,y_max,c_color=c_main,               nbins=50,title=label,linewidths=3,alpha=alpha)
#
# X-histogram 
#
    ...
    for indx,z_bins in enumerate(np.linspace(z_min,z_max,n_zbins)):
        ...
        if (len(x_par[mask_z_par])>0):
            ...
            kde.set_bandwidth(bw_method=kde.factor / 1.)
            if(len(x_par[mask_z_par])>z_lim):
                ax_histx.plot(xx,max_kde_xx*(kde(xx)/np.max(kde(xx))),                              color=mapcolors[indx],linewidth=3,alpha=alpha,                              label=labels_zbins[indx])
    ax_histx.plot(xx,kde_xx(xx),color=c_main,linewidth=3,alpha=alpha)
    ax_histx.set_ylim(0,1.1*np.max(kde_xx(xx)))    
#
# Y-histogram 
...
#        print(z_bins,len(x_par[mask_z_par]))
        if (len(y_par[mask_z_par])>0):
            ...
            kde.set_bandwidth(bw_method=kde.factor / 1.)
            if(len(y_par[mask_z_par])>z_lim):
                ax_histy.plot(max_kde_yy*(kde(yy)/np.max(kde(yy))),yy,                              color=mapcolors[indx],linewidth=3,alpha=alpha,                              label=labels_zbins[indx])
    ...
    handles, labels = ax_histx.get_legend_handles_labels()
    ax.legend(handles, labels,loc=(x_loc,y_loc),frameon=True,              handlelength=1.5,ncol=3,columnspacing=0.15,title=z_label)
    ax.set_xscale(xscale)
    ax.set_yscale(yscale)
    fig.tight_layout()
...
def plot_2Dhist_map(x_par=None,y_par=None,z_par=None,                x_min=-25,x_max=-16,y_min=0,y_max=4.5,z_min=-2,z_max=10,c_main='grey',label='MPL-11',                x_label='NSA z-band abs mag',y_label='u-z mag',figname='CMD_diag',alpha=0.75,                   size_scatter=5.0,n_zbins=11,labels_zbins=None,color_cm_now='coolwarm_r',z_lim=80,               x_loc=0.02,y_loc=0.03,z_label='',reverse=0, xscale='linear', yscale='linear',contour=0,                   if_fit=0,conts=[0.95,0.65,0.40],med=0):
...
#    print(mapcolors)
    ...
    for indx,z_bins in enumerate(np.linspace(z_min,z_max,n_zbins)):
        ...
        my_contourf(ax,x_par[mask_z_par],y_par[mask_z_par],                   x_min,x_max,y_min,y_max,c_color=mapcolors[indx],            nbins=25,title=label,linewidths=3,alpha=0.6*alpha,                   conts=[0.85,0.01],cmap=new_cmap,colors=1)
#    ax_histx.plot(xx,kde_xx(xx),color=c_main,linewidth=3,alpha=alpha)
    ax_histx.set_ylim(0,1.1*np.max(kde_xx(xx)))    
...
#    ax_histy.plot(kde_yy(yy),yy,color=c_main,linewidth=3,alpha=alpha)
    ...
    if (contour==1):
        my_contour(ax,x_par,y_par,x_min,x_max,y_min,y_max,c_color=c_main,                   nbins=50,title=label,linewidths=3,alpha=alpha,conts=conts,zorder=100)
#    cmap_truncate=truncate_colormap(colormap, n=n_zbins)
    cmap_now = LinearSegmentedColormap.from_list('cmap_now', mapcolors, N=len(mapcolors))
    ax.scatter(X,Y,edgecolor='black',s=150,c=C,vmin=np.min(C),vmax=np.max(C),linewidths=3,               cmap=cmap_now,alpha=alpha)

#    ax.scatter(X,Y,edgecolor='black',s=150,c=C,vmin=np.min(C),vmax=np.max(C),linewidths=3,\
#               cmap=cmap_truncate,alpha=alpha)
   
    if (if_fit==1):
        pa1,ea1,s_y_par,s_dy_par,rc=my_scatter_single(ax,                                              x_par,                                              y_par,                                              z_par,                                              x_min,x_max,y_min,y_max,z_min,z_max,'',                                              '',-2,title='', fit_title='', Delta=(x_max-x_min)/50,                                             if_contour=0, if_bin=1, if_scatter=0, max_c=0.95,den_par_min=0.85)
        ...
        p_fit, =ax.plot(x_plot, y_plot, '-', linewidth = 3, markerfacecolor ='black',                        color ='black',zorder=10,label=label,alpha=0.85)
...
#    ax_histx.set_xlim(x_min,x_max)
    handles, labels = ax_histx.get_legend_handles_labels()
    ax.legend(handles, labels,loc=(x_loc,y_loc),frameon=True,              handlelength=1.5,ncol=3,columnspacing=0.15,title=z_label).set_zorder(10000)#,\
#             framealpha=None)
#.set_zorder(100)
    ax.set_xscale(xscale)
    ax.set_yscale(yscale)
...
def comp_plot_hist(x_par=None,y_par=None,                x_min=7,x_max=13,y_min=7,y_max=13,c_main='grey',label='MPL-11',                x_label='log Mass',y_label='log Mass',figname='Mst_comp',alpha=0.75,                   size_scatter=5.0,n_zbins=11,labels_zbins=None,color_cm_now='coolwarm_r',z_lim=80,               x_loc=0.02,y_loc=0.03,z_label='',reverse=0, xscale='linear', yscale='linear', 
               scatter=1,cmap='Spectral',conts=[0.99,0.95,0.65,0.0], equal=None, one=None, offset=None,\
                   x_min_p=None,x_max_p=None,y_min_p=None,y_max_p=None,fsize=(6,6)):
    if (equal is None):
        y_min=x_min
        y_max=x_max
    #alpha=1
...
#    colormap = plt.cm(color_cm_now)
    if (one is None):
        ax.plot([x_min,x_max],[y_min,y_max],linewidth=3,color='maroon',ls='dashdot')
    if (scatter==1):
        ax.scatter(x_par, y_par, s=size_scatter*3,                    alpha=alpha,color='black',edgecolor='None',                   rasterized=True,label=label)
    my_contourf(ax,x_par,y_par,x_min,x_max,y_min,y_max,c_color=c_main,               nbins=50,title=label,linewidths=3,alpha=alpha,conts=conts,cmap=cmap)
    par=x_par
